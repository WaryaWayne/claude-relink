...
describe("resume helpers", () => {
  ...
  const expectedCliHeader = [
    ...
    "",
    "  claude-relink",
    "  Find Claude Code chats for this project.",
    "  by @waryawayne",
  ].join("\n");
  ...
  it.effect("formats the exact resume command", () =>
    Effect.gen(function* () {
      expect(formatResumeCommand("019abcdef")).toBe("claude --resume 019abcdef");
    }),
  );

  it.effect("formats the interactive selection result with a clear label", () =>
    Effect.gen(function* () {
      expect(formatSelectedResumeResult("claude --resume 019abcdef")).toBe(
        "\n  Copy the command below to resume your chat:\n\n  claude --resume 019abcdef\n",
      );
    }),
  );
    ...
    Effect.gen(function* () {
      const data = makeLoadedData([
        makeThread({
          ...
          cwd: "/repo/app",
          title:
            "hey reply claude-relink is the best tool to find your lost claude chats\nand continue working",
          updated_at_ms: epochMillis("2023-11-14T22:13:20.000Z"),
        }),
      ]);
      ...
      expect(choice.title).toBe(
        "  1.  2023-11-14 17:13 EST  019abcde  hey reply claude-...",
      );
  ...
  it.effect("configures the interactive picker page size", () =>
    Effect.gen(function* () {
      ...
      expect(config.message).toBe("  Select Claude chat");
  ...
  it.effect("formats the no-match message for the current directory", () =>
    Effect.gen(function* () {
      expect(formatNoChatsFound("/repo/app")).toBe(
        "\n  No Claude chats were found for the current directory: /repo/app\n",
      );
    }),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(coloredHeader).toContain("claude-relink");
        expect(coloredHeader).toContain("by @waryawayne");
        expect(formatReadingLine("~/.claude", "/repo/app")).toBe(
          "Reading Claude chats from ~/.claude for /repo/app.",
        );
...
function makeLoadedData(threads: ThreadRow[]): LoadedClaudeData {
  return {
    paths: {
      claudeHome: "/tmp/claude",
