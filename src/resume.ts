...
class NonInteractiveTerminal extends Data.TaggedError(
  "NonInteractiveTerminal",
)<{}> {
  override get message(): string {
    return "claude-relink list requires an interactive TTY for stdin and stdout.";
  }
}

export function findResumeCandidates(
  data: LoadedClaudeData,
  cwd: string,
...
export function createResumePromptConfig(
  ...
  return {
    message: `${CLI_BLOCK_INDENT}Select Claude chat`,
...
export function formatNoChatsFound(cwd: string): string {
  return formatCliOutputBlock([
    `No Claude chats were found for the current directory: ${cwd}`,
  ]);
}

export function formatCliHeader(
  options: { claudeHome?: string; color?: boolean } = {},
): string {
  const name = "claude-relink";
  const findLine = "Find Claude Code chats for this project.";
  ...
  if (options.color === true) {
    return [
      ...
      `${CLI_HEADER_INDENT}${ANSI_CYAN_BRIGHT}Find${ANSI_RESET} Claude Code chats for this project.`,
...
export function formatReadingLine(claudeHome: string, cwd: string): string {
  return `Reading Claude chats from ${claudeHome} for ${cwd}.`;
}

export function formatUnknownSubcommandError(subcommand: string): string {
  return formatCliOutputBlock([
    `Error: unknown subcommand "${subcommand}" for "claude-relink".`,
    `Use "claude-relink --help" to see available commands: latest, list.`,
  ]);
}
...
function getThreadResumeTime(thread: ThreadRow): number {
  if (
...
function renderResumeSelectPrompt(
  ...
  return [
    `${ANSI_CURSOR_HIDE}${PROMPT_LEADING_SYMBOL} ${CLI_BLOCK_INDENT}Select Claude chat ${PROMPT_TRAILING_SYMBOL}`,
    ...rows,
  ].join("\n");
}
