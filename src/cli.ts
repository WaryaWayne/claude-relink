...
const FLAGS_WITH_VALUE = new Set(["--claude-home", "--log-level", "--completions"]);
...
const claudeHome = Flag.string("claude-home").pipe(
  Flag.withDescription("Claude home directory. Defaults to ~/.claude."),
  Flag.withDefault("~/.claude"),
);

const root = Command.make("claude-relink").pipe(
  Command.withSharedFlags({ claudeHome }),
  Command.withDescription(
    "Find Claude Code chats for the current directory and print resume commands.",
  ),
);
...
const latest = Command.make(
  ...
  Effect.fn("Cli.latest")(function* () {
    ...
    const candidates = yield* loadResumeCandidates(options.claudeHome, cwd);
  ...
  Command.withDescription(
    "Print the newest Claude resume command for the current directory.",
  ),
);
...
const list = Command.make(
  ...
  Command.withDescription(
    "Pick a Claude chat for the current directory and print its resume command.",
  ),
);
...
function loadResumeCandidates(claudeHome: string, cwd: string) {
  return Effect.gen(function* () {
    yield* Console.error(formatCliHeader({ claudeHome, color: stderrSupportsColor() }));
    const readingLine = formatReadingLine(claudeHome, cwd);
    const data = yield* withStderrSpinner(readingLine, loadClaudeData({ claudeHome }));
...
function withStderrSpinner<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    ...
    () => effect,
    stopStderrSpinner,
  ).pipe(Effect.tap(() => Console.error(`${STATUS_LINE_INDENT}${message}`,formatCompletedStatusLine(message))));
}
...
function getUnknownSubcommand(args: readonly string[]): string | null {
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg.startsWith("--claude-home=") || arg.startsWith("--log-level=")) {
      continue;
    }
